...
export default function BrandingPage() {
  return (
    <main className="relative bg-bone">
      ...
      {BRANDS.map((b, i) => (
        <section key={b.slug} className="border-t border-ink/15 px-[3vw] py-[12vh]">
          <div className="mx-auto max-w-6xl">
            <div className="flex items-baseline justify-between font-mono text-[10px] uppercase tracking-[0.35em] text-ink/50">
              <span>{b.sector}</span>
            ...
            <p className="mt-2 font-mono text-[11px] uppercase tracking-[0.25em] text-ink/60">{b.tagline}</p>

            {/* the sketch, centred first */}
            <figure className="mx-auto mt-12 max-w-3xl">
              <ZoomImage src={b.sketch} alt={`${b.name} exploration sketch`} caption={`${b.name} — exploration`} className="fibre bg-white border border-ink/20" />
            ...
            <div className="mt-16 grid gap-[5vh] lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)] lg:gap-[4vw]">
              <ZoomImage src={b.board} alt={`${b.name} brand system`} caption={`${b.name} — brand system`} className="fibre bg-white border border-ink/20 lg:order-1" />
              <div className="lg:order-2 lg:sticky lg:top-[12vh] lg:h-fit">
                ...
                <p className="mt-4 text-[clamp(0.95rem,1.3vw,1.15rem)] leading-relaxed text-ink/75">{b.blurb}</p>
                <div className="mt-6 flex items-center gap-2">
                  {b.palette.map((c) => (
                    <span key={c} className="h-8 w-8 border border-ink/20" style={{ background: c }} title={c} />
                  ))}
                </div>
              </div>
            </div>
      ...
      <footer className="border-t border-ink/15 px-[4vw] py-[14vh] text-center">
        ...
        <Link href="/work/posters" className="mt-4 inline-block font-display text-[clamp(1.8rem,7vw,4.5rem)] uppercase text-ink hover:text-red">
          Posters →
        </Link>
      </footer>
    </main>
