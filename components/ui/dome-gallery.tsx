...
// reactbits DomeGallery — 3D draggable sphere of images. Verbatim, typed loosely.
...
export default function DomeGallery({
  images = [],
  fit = 0.6,
  fitBasis = "auto" as "auto" | "min" | "max" | "width" | "height",
  minRadius = 600,
  maxRadius = Infinity,
  padFactor = 0.25,
  overlayBlurColor = "#060507",
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const orig = originalTilePositionRef.current;
      if (overlay && orig) {
        overlay.style.transition = `all ${enlargeTransitionMs}ms ease`;
        const mainR = mainRef.current!.getBoundingClientRect();
        overlay.style.left = `${orig.left - mainR.left}px`;
        overlay.style.top = `${orig.top - mainR.top}px`;
        overlay.style.width = `${orig.width}px`;
        overlay.style.height = `${orig.height}px`;
        overlay.style.opacity = "0";
        setTimeout(() => overlay.remove(), enlargeTransitionMs + 30);
      }
      refDiv?.remove();
