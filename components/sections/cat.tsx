...
export function Cat() {
  ...
  return (
    <section className="on-dark grid-lines-dark relative overflow-hidden border-y border-bone/10 py-[10vh]">
      <div className="mx-auto max-w-6xl px-[5vw]">
        <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-yellow">the easter egg</p>
        <h2 className="mt-2 font-condensed text-[clamp(2.5rem,9vw,6rem)] uppercase leading-none">Move the cat</h2>

        <div
          className="relative mt-8 aspect-[16/9] w-full select-none"
          onPointerMove={(e) => {
        ...
        >
          ...
          {!ready && (
            <span className="absolute inset-0 grid place-items-center font-mono text-[10px] uppercase tracking-[0.3em] text-bone/40">
              loading frames…
            </span>
          )}
          <span className={`pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 font-mono text-[10px] uppercase tracking-[0.3em] transition-opacity ${hint === "left" ? "opacity-100 text-yellow" : "opacity-30 text-bone"}`}>
            ← walk left
          </span>
          <span className={`pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 font-mono text-[10px] uppercase tracking-[0.3em] transition-opacity ${hint === "right" ? "opacity-100 text-yellow" : "opacity-30 text-bone"}`}>
            walk right →
          </span>
        </div>

        <div className="mt-4 flex gap-3 font-mono text-[10px] uppercase tracking-[0.25em] text-bone/50">
